        ...
        <>
            ...
            <main>
                ...
                <section id={`contact`} className={`contactContentSection flex alignCenter justifyCenter flexColumn`} style={{padding: 15}}>
                    ...
                    <form id={`contactForm`} className={`flex flexColumn gap5`} onInput={() => contactFormInput()} onSubmit={(e) => contactFormSubmission(e)}>

                        <input onFocus={() => setEmailTouched(true)} onInput={(e) => setEmail(e.target.value)} id={`contactForm_email`} name={`email`} type={`email`} className={`contactFormField`} placeholder={`Enter Email Address...`} required />
